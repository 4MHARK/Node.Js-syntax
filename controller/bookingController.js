...
const bookingController = {
    getNewIdReservation:  async (req, res) => {
        try {
      ...
      getremoveById : async (req, res) => {
        ...
        try {
          ...
          if (itemIndex > -1) {
            ...
            if (
              typeof cart.totalRoom === "number" &&
              !isNaN(cart.totalRoom) &&
              typeof item.noRoom === "number" &&
              !isNaN(item.noRoom)
            ) {
              cart.totalRoom -= item.noRoom;
            }
            if (
              typeof cart.totalCost === "number" &&
              !isNaN(cart.totalCost) &&
      ...
      getcheckingByorderByIdByitemIdbyroom:  async (req, res) => {
        try {
          const errorMsg = req.flash("error")[0];
          const successMsg = req.flash("success")[0];
      ...
      postcheckingByrooms : async (req, res) => {
        const {
          identification,
          denomination,
          religion,
      ...
      getcheckingBysucessByorderIdByitemId:  async (req, res) => {
        const orderId = req.params.orderId; // Access the order ID
        const itemId = req.params.itemId; // Access the item reference
      ...
      getcheckingBysucessByorderIdByitemIdBypdf : async (req, res) => {
        try {
          ...
          const filteredReservations = guest.reservations.filter(
            (reservation) => reservation.itemId.toString() === itemId.toString()
          );
      ...
      postreserveBytypeByid : async (req, res) => {
        ...
        try {
          ...
          if (cartItem) {
            ...
            genericItem = {
              ...genericItem,
              ...calculateTypeSpecificAttributes(item, bookingData),
            };
          ...
          req.flash("success", "Item added to the shopping cart");
          res.json({
            success: "Item added to the shopping cart.",
            redirectUrl: "/bookings/cart",
          });
        } catch (err) {
          res.json({ error: err.message });
...
module.export = bookingController;
